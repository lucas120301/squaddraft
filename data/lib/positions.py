"""FBref position -> game position mapping and fit generation."""
...
# FBref coarse codes — combos map to realistic roles (DF,FW = wide attacker, not CB).
FBREF_TO_GAME = {
    "GK": ("GK", []),
    "DF": ("CB", ["LB", "RB"]),
    "MF": ("CM", ["CAM", "DM"]),
...
def is_valid_player_name(name: str, *, minutes: int = 0) -> bool:
    """Full names always pass; mononyms pass if they have real minutes in this era."""
...
def resolve_era_position(
    ...
    *,
    ...
    """Map FBref code + era stats to a game role.

    FBref's plain ``DF`` lumps centre-backs and full-backs; plain ``FW`` lumps
    strikers and wingers. Use per-era output to disambiguate.
    """
    pos = (fbref_pos or "").strip().upper()
    primary, secondaries = parse_fbref_positions(pos)

    if pos == "DF":
        # Full-backs create chances; CBs who score headers can have high G+A with few assists.
        if ast_per90 >= 0.06:
            return _side_fullback(player)
        # Stat-less but substantial DF minutes in an era → centre-back (sparse G+A CBs).
        if ga_per90 == 0 and gls_per90 == 0 and ast_per90 == 0 and minutes >= 900:
            return "CB", ["LB", "RB"]
        # Low-scoring, low-assist defender with real minutes → centre-back.
        if gls_per90 <= 0.03 and 0.02 <= ga_per90 < 0.15:
            return "CB", ["LB", "RB"]
        # CBs who chip in with occasional headers (van Dijk, Terry) — few assists.
        if (
            ast_per90 < 0.05
            and gls_per90 <= 0.22
            and ga_per90 < 0.25
    ...
    if pos == "FW":
        if gls_per90 >= 0.55:
            return "ST", ["LW", "RW", "CAM"]
        # Breakthrough / counter-attacking striker (Vardy 2010s) — high assists, lower goals/90.
        if (
            gls_per90 >= 0.15
            and ast_per90 >= 0.20
...
def position_weights(primary: str) -> tuple[float, float, float, float]:
    """attack, midfield, defence, gk weights for base_rating."""
    if primary == "GK":
        return 0.05, 0.10, 0.15, 0.70
    if primary in {"ST", "LW", "RW"}:
        return 0.55, 0.25, 0.15, 0.05
    if primary in {"CB", "LB", "RB", "LWB", "RWB"}:
        return 0.15, 0.25, 0.55, 0.05
    if primary in {"DM", "CM", "CAM", "LM", "RM"}:
        return 0.25, 0.50, 0.20, 0.05
